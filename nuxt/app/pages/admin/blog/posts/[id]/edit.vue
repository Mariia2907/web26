<template>
  <div class="container mx-auto px-4 py-8 max-w-2xl">
    ...
    <div v-if="loadingPost" class="flex justify-center py-20">
    ...
    <div v-else class="bg-white border border-gray-200 rounded-xl p-6">
      <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
        ...
        <UFormField label="Slug" name="slug">
          ...
          />
          <template #hint>
            <span class="text-xs text-gray-400">Очисти щоб згенерувати новий з заголовку</span>
          </template>
        </UFormField>
...
const getPost = async () => {
  ...
  try {
    ...
    state.category_id  = post.category_id  ?? undefined
    state.excerpt = post.excerpt ?? ''
    state.content_raw  = post.content_raw  ?? ''
    state.is_published = post.is_published ?? false
  } catch {
    toast.add({ title: 'Помилка', description: 'Не вдалось завантажити пост', color: 'error' })
...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  ...
  try {
    ...
    toast.add({
      title: 'Успішно',
      description: res.message ?? 'Пост оновлено',
      color: 'success',
    })
    router.push('/admin/blog/posts')
  } catch (err: any) {
...
await Promise.all([getPost(), getCategories()])
</script>
